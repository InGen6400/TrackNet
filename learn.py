...
def create_data(file, frame_length: int):
    df = pd.read_csv(file)
...
def param_model():
    ...
    (train_X, train_Y), (test_X, test_Y) = create_data(sys.argv[1], frame)
    ...
    filepath = './saves/model_{}_{}_{}_{}_{}'.format(frame, hide_num, hide_unit, dense_unit, lr) + \
               '_{epoch:02d}.hdf5'
    ...
    cp = ModelCheckpoint(filepath=filepath, monitor='loss', verbose=1, save_best_only=True, mode='auto')
