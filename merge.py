...
if __name__ == '__main__':
    ...
    sensor_files = glob.glob("H:\Research_Resource/3/sen*.csv")
    position_files = glob.glob("H:\Research_Resource/3/pos*.csv")
    sensor_files.sort()
    position_files.sort()
    ...
    for sensor_file, position_file in zip(sensor_files, position_files):
        ...
        out_frame.columns = ['dt[s]', 'sensor_dt[s]', 'pos-sens_delta[ms]',
                             ...
                             'pos_x', 'pos_y', 'pos_z',
                             'rot_x', 'rot_y']
        out_frame.to_csv('merged/'+os.path.basename(position_file) + OUTPUT + ".csv", index=False)
